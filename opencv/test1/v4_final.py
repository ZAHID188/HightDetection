...
# Initialize webcam
cap = cv2.VideoCapture(1)  # Changed to 0 to address the camera access error
...
# Static parameters (based on the values from your image)
CANNY_THRESHOLD1 = 30
...
scale_range = 15  # Default scale range in cm (from your image)
...
# Create window and add only the scale range trackbar
...
while True:
    ...
    # Get only the scale range trackbar value
    ...
    # 4. Apply Canny edge detection with static parameters
    ...
    # Use Hough Line Transform to detect lines with static parameters
    ...
    # Draw the scale using our function
    ...
    if key == ord('q'):  # Quit
        ...
        scale_range = 15  # Reset to the value from your image
        cv2.setTrackbarPos('Scale Range (cm)', 'Height Measurement (cm)', scale_range)
